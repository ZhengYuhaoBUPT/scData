...
class DiffusionHeadConfig(PretrainedConfig):
    def __init__(self, 
        ...
        """
        Args:
            hidden_size: 必须与 LLM (Qwen2.5-7B) 的 hidden_size 一致 (默认 3584)
            num_attention_heads: 注意力头数 (默认 28，对应 Qwen2.5-7B)
            head_dim: hidden_size // num_attention_heads (默认 128)
            intermediate_size: FFN 中间层维度 (默认 18928，对应 Qwen2.5-7B)
            num_layers: 扩散层数量 (默认 6)
        """
        self.attn_implementation = 'sdpa'
        self.max_position_embeddings = 131072
        ...
        self.num_key_value_heads = 4  # GQA 配置，参考qwen配置设置
        self.hidden_act = 'silu'
        self.rms_norm_eps = 1e-05
        self.mlp_bias = False
        ...
        self.qk_norm = True
        self.num_layers = num_layers  # ✅ 保存层数配置
        super().__init__()
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        # 🔧 关键修复：处理双维 position_ids [B, 2, S] 或 [2, B, S]
        # 对于 diffusion head，使用文本轴（第一维）
        if position_ids.dim() == 3:
            if position_ids.shape[1] == 2:
                # [B, 2, S] -> 使用文本轴 [B, S]
                position_ids = position_ids[:, 0, :]
            elif position_ids.shape[0] == 2:
                # [2, B, S] -> transpose 并使用文本轴 [B, S]
                position_ids = position_ids[0, :, :]

        if "dynamic" in self.rope_type:
            self._dynamic_frequency_update(position_ids, device=x.device)

        # ✅ 致命修复：确保 inv_freq 在正确的设备上（防止 DeepSpeed ZeRO-3 导致的设备不匹配）
        if self.inv_freq.device != x.device:
            self.inv_freq = self.inv_freq.to(x.device)
            if hasattr(self, 'original_inv_freq'):
                self.original_inv_freq = self.original_inv_freq.to(x.device)
