...
LOG_FILE="$PROJECT_ROOT/run/logs/stage1.log"
...
if [ "$MODE" == "foreground" ] || [ "$MODE" == "fg" ] || [ "$MODE" == "1" ]; then
    ...
    accelerate launch \
        ...
        "$PROJECT_ROOT/src/train/train_stage1_bidirectional_rank_sin.py" 2>&1 | tee -a "$LOG_FILE"
else
    echo "🚀 以后台模式启动..."
    echo ""
    ...
    nohup accelerate launch \
        --config_file "$PROJECT_ROOT/accelerate_configs/sc_showo_multi_gpu_deepspeed.yaml" \
        --main_process_port=19999 \
        "$PROJECT_ROOT/src/train/train_stage1_bidirectional_rank_sin.py" > "$LOG_FILE" 2>&1 &
