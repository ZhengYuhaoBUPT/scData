#!/bin/bash
# ============================================================================
# 🚀 SC-Show-O CW Ablation Stage 2 (Cell-only) - 一键启动脚本
# ============================================================================
# 使用方法：
#   bash src_ablation_cw/train/train_stage2_cw.sh
#   bash src_ablation_cw/train/train_stage2_cw.sh fg
# ============================================================================
...
SCRIPT_PATH="$PROJECT_ROOT/src_ablation_cw/train/train_stage2_cw_cell_only_lora.py"

echo ""
echo "╔════════════════════════════════════════════════════════════════════╗"
echo "║        CW Ablation Stage 2 (Finetune + 10k Pretrain Mix)          ║"
echo "╚════════════════════════════════════════════════════════════════════╝"
echo ""
...
if [ "$MODE" == "foreground" ] || [ "$MODE" == "fg" ] || [ "$MODE" == "1" ]; then
  echo "🚀 以前台模式启动（实时日志）..."
  cd "$PROJECT_ROOT"
  accelerate launch     --config_file "$ACCEL_CONFIG"     --main_process_port=19992     "$SCRIPT_PATH" --config "$CONFIG_PATH" 2>&1 | tee -a "$LOG_FILE"
else
  echo "🚀 以后台模式启动..."
  cd "$PROJECT_ROOT"
  nohup accelerate launch     --config_file "$ACCEL_CONFIG"     --main_process_port=19992     "$SCRIPT_PATH" --config "$CONFIG_PATH" > "$LOG_FILE" 2>&1 &
