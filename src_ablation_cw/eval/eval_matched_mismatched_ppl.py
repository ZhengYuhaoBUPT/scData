...
def parse_args():
    ...
    parser.add_argument('--use-system-metadata', action='store_true', default=False)
    parser.add_argument('--eval-scope', type=str, choices=['easy', 'full'], default='full', help='easy: only easy QA turns; full: all QA turns')
    parser.add_argument(
    ...
    parser.add_argument('--use-explicit-mask', action='store_true',
                        help='Use explicit omni-attn mask instead of standard causal mask. Default is False to align with train.')
    ...
    parser.add_argument(
        ...
        default='cell_only',
        help='For cell-only ablation, gene_only/both are treated as cell_only.',
    )
    parser.add_argument(
        '--avoid-same-answer',
        action='store_true',
...
def is_easy_qa(question: str, answer: str) -> bool:
    ...
    if len(q_words) > 30 or total_words > 180: return False

    starts_ok = q.startswith(("what", "which", "is this", "identify", "name", "describe", "tell me"))
    has_question_mark = "?" in q
    if not (starts_ok or has_question_mark): return False
    ...
    easy_signals = [
        "what type", "what kind", "cell type", "what cells", "which cell type", "identify",
        "describe the cell", "describe these cells", "what is the cell",
        "tell me about the cell type", "what can you tell me about the cell types"
    ]
    if not (any(sig in q for sig in easy_signals) or (has_cell and has_type_intent)): return False

    complex_keywords = [
        "pathway", "pathways", "function", "functions", "mechanism", "mechanisms", "regulation",
        "signal transduction", "gene ontology", "go term", "enrichment", "metabolic", "immune response",
        "differential", "trajectory",
    ]
    if any(k in q for k in complex_keywords): return False
    return True
...
def collect_eval_cases(all_items, easy_only: bool = True):
    ...
    for item in all_items:
        ...
        if not cell_id or not conv: continue

        for i in range(len(conv) - 1):
            if conv[i].get('from') != 'human' or conv[i + 1].get('from') != 'gpt': continue
            q = conv[i].get('value', '')
            a = conv[i + 1].get('value', '')
            if easy_only and (not is_easy_qa(q, a)): continue
...
def main():
    ...
    if args.eval_scope == 'easy':
        ...
        if not selected_cases: raise ValueError('No EASY QA turns matched the heuristic filter.')
    else:
        selected_cases = all_cases
        if not selected_cases: raise ValueError('No QA turns found in qa-jsons.')
    ...
    pool_ids = list(dict.fromkeys(str(item.get('id')) for item in all_items if item.get('id') is not None))
    ...
    for item in all_items:
        cid = str(item.get('id'))
        conv = item.get('conversations', [])
        if not cid or not conv: continue
    ...
    for item_idx, item in enumerate(items):
        ...
        matched_payload = [build_stage2_eval_sample(true_cell, conversations, tokenizer, config, include_system_metadata=args.use_system_metadata, append_image_tag=args.append_image_tag)]
        matched_ppl = batched_ppl(model, tokenizer, args.device, 1, matched_payload, use_explicit_mask=args.use_explicit_mask)[0]
        ...
        if len(candidate_ids) < args.num_negatives:
            if args.stratify_negatives_by_id_kind:
                raise ValueError(
                    ...
                    f'cell_id={cell_id}, pool={len(candidate_ids)}, need={args.num_negatives}. '
                    f'Use --allow-negative-replacement or disable --stratify-negatives-by-id-kind.'
                )
            if not args.allow_negative_replacement:
                raise ValueError(
                    f'Not enough negatives for cell {cell_id}: pool={len(candidate_ids)} < num_negatives={args.num_negatives}. '
                    f'Pass --allow-negative-replacement to sample with replacement.'
                )
        ...
        for mismatch_id in mismatch_ids:
            ...
            mismatch_payloads.append(build_stage2_eval_sample(mixed_cell, conversations, tokenizer, config, include_system_metadata=args.use_system_metadata, append_image_tag=args.append_image_tag))

        mismatch_ppls = batched_ppl(model, tokenizer, args.device, args.batch_size, mismatch_payloads, use_explicit_mask=args.use_explicit_mask)
    ...
    metrics = {
        ...
        'mean_quantile': statistics.mean(quantiles),
        'top1_match_rate': statistics.mean(top1_hits),
        ...
        'notes': 'Matched keeps true cell+QA; mismatched swaps conditioning cell only (cell-only ablation).',
    }
